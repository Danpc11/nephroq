...
with st.sidebar:
    ...
    st.divider()
    treated = st.checkbox("Already receiving SGLT2i or ACEi/ARB", value=False)
...
stage = ("G1", "G2", "G3", "G4", "G5") [
    0 if egfr0>=90 else 1 if egfr0>=60 else 2 if egfr0>=45 else 3 if egfr0>=15 else 4]
col2.metric("Approximate KDIGO stage", stage)

# ---- projection ----
def project(a1c, sbp, uacr, egfr0, treated, years=15):
    orig = mt.metabolic_insult
    mt.metabolic_insult = lambda a1c,uacr,sbp: (
        W_POP[0]*max(a1c-6.5,0) + W_POP[1]*np.log1p(uacr/30) + W_POP[2]*max(sbp-130,0)/10)
    m = MechanisticRenalModel(a1c=a1c, sbp=sbp, uacr=uacr, u=1.0 if treated else 0.0,
                              k_hf=KHF_POP, q=Q_POP)
    mt.metabolic_insult = orig
...
label_b = "If SGLT2i/ACEi-ARB is added" if not treated else "If treatment is stopped"

col3.metric(f"Estimated time to dialysis ({'current' if treated else 'untreated'})",
...
ax.text(0.3, DIALYSIS_eGFR + 2, "dialysis threshold", fontsize=9)
ax.set_xlabel("years"); ax.set_ylabel("projected eGFR (mL/min/1.73m²)")
ax.set_title("Renal function projection"); ax.legend()
st.pyplot(fig)

if np.isfinite(td_a) and np.isfinite(td_b):
    diff = abs(td_b - td_a)
    st.info(f"**{label_b}** would delay (or bring forward) dialysis by approximately "
           f"**{diff:.1f} years** relative to the current scenario, according to the model.")
