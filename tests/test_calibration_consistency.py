...
def _cohort_from_model(n_pat, span, n_visits, noise, seed):
    """Virtual patients generated BY the model, so the true parameters are known."""
    rng = np.random.default_rng(seed)
    P = []
    for i in range(n_pat):
        nv = int(rng.integers(*n_visits))
        t = np.sort(rng.uniform(0, span, nv)); t[0] = 0.0
        e0 = float(rng.uniform(30, 90)); a1c = float(rng.uniform(6.5, 10))
        ua = float(np.exp(rng.uniform(3.5, 6.5))); sbp = float(rng.uniform(125, 160))
        p = dict(core.TRIAL_CALIBRATION_V2)
        e = core.predict_egfr_at_v2(e0, a1c, ua, sbp, 0.0, p, t) + rng.normal(0, noise, nv)
        P.append(dict(patient_id=str(i), t=t, e=np.clip(e, 4, None), egfr0=e0,
                      hba1c_baseline_strict=a1c, uacr_baseline_strict=ua,
                      sbp_baseline_strict=sbp,
                      hba1c_series=np.full(nv, a1c), uacr_series=np.full(nv, ua),
                      sbp_series=np.full(nv, sbp)))
    return P


def test_kfold_flags_an_unidentifiable_cohort():
    """
    Short follow-up + high noise cannot pin the parameters down. K-fold must SAY
    so: refitting on different patient subsets makes q swing across its whole
    range. A bootstrap cannot detect this -- it resamples the same patients -- so
    this is the check that stops a tight bootstrap interval from being reported as
    precision it does not have.
    """
    import calibrate_mimic as cal

    bad = _cohort_from_model(40, span=1.2, n_visits=(4, 7), noise=9.0, seed=1)
    cv = cal.cross_validate(bad, k=4, noise_sd=8.7, verbose=False)
    assert cv is not None
    assert "q" in cv["unstable"]


def test_kfold_catches_a_parameter_pinned_at_its_bound():
    """
    THE DANGEROUS FALSE GREEN. When the data carry no information about a
    parameter, the optimizer slams it into the boundary in EVERY fold. Its spread
    is then ~0, which a naive stability check reports as rock-solid. A parameter
    sitting on its bound is degenerate, not identified -- it must be flagged.
    """
    import calibrate_mimic as cal

    bad = _cohort_from_model(40, span=1.2, n_visits=(4, 7), noise=9.0, seed=1)
    cv = cal.cross_validate(bad, k=4, noise_sd=8.7, verbose=False)
    assert "q" in cv["at_bound"]                   # pinned at the lower bound
    assert cv["summary"]["q"]["cv"] < 0.05         # ...and its spread LOOKS perfect
    assert "q" in cv["unstable"]                   # ...but it is still flagged


def test_kfold_accepts_an_identifiable_cohort():
    """Long follow-up + low noise: the same machinery must NOT cry wolf."""
    import calibrate_mimic as cal

    good = _cohort_from_model(40, span=8.0, n_visits=(8, 14), noise=3.0, seed=0)
    cv = cal.cross_validate(good, k=4, noise_sd=8.7, verbose=False)
    assert cv is not None
    assert "q" not in cv["unstable"]
    assert cv["summary"]["q"]["cv"] < 0.20


def test_kfold_folds_are_disjoint_by_patient():
    """Out-of-fold error is only meaningful if the fold's patients were never seen
    during that fold's fit. Splits are by PATIENT, never by row."""
    import calibrate_mimic as cal

    pats = _cohort_from_model(24, span=5.0, n_visits=(5, 9), noise=4.0, seed=3)
    cv = cal.cross_validate(pats, k=4, noise_sd=8.7, verbose=False)
    total_test = sum(f["n_test"] for f in cv["folds"])
