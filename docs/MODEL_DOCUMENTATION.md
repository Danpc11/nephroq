...
identified.** Result from the `noise_identifiability.py` experiment:
...
|---|---|---|---|
| **Per-patient** | `inverse_fit.py` | patient with many visits (>15) | individual parameters |
| **Amortized AI** | `amortized_ai.py` | instant inference; scarce data | fast estimate + uncertainty |
| **Hierarchical** | `hierarchical_model.py` | full cohort, scarce data | *partial pooling*: patients with little data lean on the population |
...
`python mechanistic_twin.py` → eGFR trajectories (mechanistic vs linear) and
...
`python hybrid_twin.py` → linear state-space + absorbing Markov chain; time
...
`python inverse_fit.py` → recovers parameters from synthetic data.
...
`python noise_identifiability.py` → quantifies how much `q` improves with
cystatin. Justifies the assay choice.

**Step 6 — Amortized AI estimator.**
`python amortized_ai.py` → trains the network ensemble (trajectory →
...
`python hierarchical_model.py` → mixed effects with *partial pooling*.
...
`CKD_CSV=my_data.csv python hierarchical_model.py`. The `load_real_data()`
...
| `inverse_fit.py` | Inverse problem; parameter recovery (synthetic) |
| `noise_identifiability.py` | Effect of the assay (noise) on the identifiability of `q` |
| `amortized_ai.py` | Amortized AI estimator for scarce data |
| `hierarchical_model.py` | Mixed effects with *partial pooling* (real-data model) |
