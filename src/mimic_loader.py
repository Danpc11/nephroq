...
def _confirmed_index(times, egfrs, confirm_days=90, max_recovery=0.30,
                     search_limit_days=730):
    """
    Pick the index date the way a nephrologist would: the first eGFR CONFIRMED to
    be the patient's real level, not a transient dip.

    THE PROBLEM THIS SOLVES. Taking the first available creatinine as baseline is
    the single largest bias in a hospital cohort. People come to hospital *because
    they feel unwell* -- dehydrated, septic, on NSAIDs, post-contrast -- and their
    creatinine rises for reasons that have nothing to do with lost nephrons. A
    patient whose kidneys are really at eGFR 70 can be recorded at 40 on the day
    they present. Feed that 40 in as a baseline and the model projects them to
    dialysis in ~12 years when in truth they will never get there. The error runs
    in the dangerous direction: it OVER-diagnoses.

    THE RULE. KDIGO does not diagnose CKD from a single creatinine either -- the
    abnormality must PERSIST for >3 months. We require the same. An eGFR qualifies
    as the index only if a later measurement, at least `confirm_days` away, has not
    RECOVERED by more than `max_recovery` above it.

        eGFR 40 -> still ~40 three months later  => real. Use it.
        eGFR 40 -> back up to 70 three months on => acute episode. Skip it.

    Only UPWARD recovery disqualifies a candidate. A patient who is genuinely
    progressing will read LOWER later, and that must never be mistaken for an acute
    episode.

    Returns the index into `times`, or None if nothing can be confirmed -- such a
    patient has no usable baseline and is dropped rather than guessed at.
    """
    n = len(times)
    if n < 2:
        return None

    t = np.asarray([(x - times[0]).days for x in times], dtype=float)
    e = np.asarray(egfrs, dtype=float)

    for i in range(n):
        if t[i] > search_limit_days:
            break                      # do not wander years into the follow-up
        later = np.flatnonzero((t - t[i]) >= confirm_days)
        if later.size == 0:
            break                      # nothing far enough ahead to confirm with
        j = later[0]
        if not np.isfinite(e[i]) or e[i] <= 0 or not np.isfinite(e[j]):
            continue
        recovery = (e[j] - e[i]) / e[i]
        if recovery <= max_recovery:
            return int(i)              # it held (or fell further) -- this is real
    return None


def _first_index(times, egfrs, **kw):
    """The original rule: the first creatinine, whatever it was measured during."""
    return 0 if len(times) else None


INDEX_STRATEGIES = {"first": _first_index, "confirmed": _confirmed_index}
...
def main(mimic_dir, out_path, min_span_days=180, min_points=4,
         index_strategy="confirmed"):
    """
    index_strategy:
      "confirmed" (default) -- KDIGO-style: the index eGFR must still hold >=90 days
          later (not recovered by >30%). This is what stops an acute presentation
          from being recorded as the patient's chronic baseline.
      "first" -- the original rule (first creatinine, whatever it was drawn during).
          Kept so the AKI bias can be MEASURED by running both and comparing.
    """
    ...
    for pid, g in creat.groupby("subject_id"):
        ...
        # eGFR at every draw, needed to CHOOSE the index (see _confirmed_index)
        times_all = list(g.charttime)
        ages_all = [age + (ct.year - anchor_year) for ct in times_all]
        egfrs_all = egfr_cr(g.valuenum.to_numpy(dtype=float),
                            np.asarray(ages_all, dtype=float),
                            np.full(len(g), sex))

        i0 = INDEX_STRATEGIES[index_strategy](times_all, egfrs_all)
        if i0 is None:
            # No creatinine could be confirmed as a real baseline: every early value
            # recovered, i.e. this patient only ever presented acutely. Dropping them
            # is the honest choice -- guessing a baseline is what caused the bias.
            n_no_confirmable_baseline += 1
            continue
        if i0 != 0:
            n_index_moved += 1

        # everything before the confirmed index is an acute prelude, not follow-up
        g = g.iloc[i0:]
        if len(g) < min_points:
            n_no_confirmable_baseline += 1
            continue
    ...
    if index_strategy == "confirmed":
        print(f"      Index date = first CONFIRMED eGFR (KDIGO-style: the value must not "
              f"recover by >30% at >=90 days).")
        print(f"      Index moved off the first creatinine for {n_index_moved} patients "
              f"(their first draw was an acute episode).")
        print(f"      Dropped {n_no_confirmable_baseline} patients with NO confirmable "
              f"baseline (they only ever presented acutely).")
